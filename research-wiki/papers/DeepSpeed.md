...
- <mark>计算效率：</mark>为了在不牺牲计算效率的情况下将 worker 数量扩展至超出模型和流水线并行能支持的规模，我们使用了 ZeRO 支持的数据并行功能（ZeRO-DP）。ZeRO-DP 不仅可以通过划分优化器状态量进一步提高显存利用效率，而且还可以通过利用基于通信拓扑的映射关系，以最小的通信开销扩展到任意数量的 GPU。<small style="color:#8b949e;">张量并行受到通信带宽的限制，能在同一层划分的张量并行数是有限的。流水线并行则收到模型层数的影响也是有限的。当GPU数超过这个上限时，策略是做数据并行并且将优化器分片，只需要通过分片优化器状态消除了冗余</small>
