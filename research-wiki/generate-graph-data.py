...
function applyFilters(){
  ...
  if(selectedId){
    ...
    PAPERS.forEach(function(p){matches[p.id]=true});
  }
  ...
  document.getElementById('searchCount').textContent=selectedId?(count+' nodes'):(PAPERS.length+' papers');
  document.getElementById('graphTitle').textContent=selected?'3-Hop Neighborhood · '+selected.short:'Knowledge Graph · All Papers';
  ...
  if(window._sim)window._sim.alpha(selectedId?0.9:0.5).restart();
}
